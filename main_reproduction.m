%% 复现论文主程序 (main_reproduction.m) [语法修复版]
clear; clc; close all;
...
area_size = 1000;    
base_N = 400;        
grid_N = 1600;       % 必须足够大，以消除 Method 2 的边缘效应
...
for i = 1:length(scenarios)
    ...
    t_color = 'k';
    % 预期低分的场景 (随机/簇内随机)
    if (i==5 || i==6 || i==7) && s2 < 0.6
         t_color = [0 0.5 0];
    % 预期高分的场景 (网格/簇内有序)
    elseif (i==4 || i==8 || i==9) && s2 > 0.8
         t_color = [0 0.5 0];
    end
    
    title(sprintf('%s\nM2=%.2f | M3=%.2f', item{3}, s2, s3), 'Color', t_color, 'FontWeight', 'bold');
    fprintf('完成: %s\n', item{3});
end
